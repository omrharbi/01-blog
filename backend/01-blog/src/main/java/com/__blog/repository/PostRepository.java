...
public interface PostRepository extends JpaRepository<Post, UUID> {
        ...
        @Query("""
                        SELECT new com.__blog.model.dto.response.post.PostResponse(
                           ...
                           LEFT JOIN p.comments c
                            WHERE (p.hidden=false AND p.createdAt <= :snapshotTime )
                            GROUP BY
                           p.id,
                           p.title,
                           p.content,
        ...
        @Query("""
                        ...
                        WHERE p.hidden=false  AND  ( u.id = :currentUserId OR s.subscriberUser.id IS NOT NULL)
        ...
        @Query("""
                        ...
                        WHERE p.user.id = :userId AND p.hidden=false
                        AND (p.createdAt <= :snapshotTime)
                        ORDER BY p.createdAt DESC
                                    """)
        ...
        @Query("""
                            ...
                            GROUP BY p.id, p.title, u.firstname, u.lastname, p.createdAt,  u.role, p.hidden
