...
public interface PostRepository extends JpaRepository<Post, UUID> {
        ...
        // Get single post without media/tags (firstImage optional)
        @Query("""
                            SELECT new com.__blog.model.dto.response.post.PostResponseWithMedia(
                                p.id,
                                p.user.id,
                                p.title,
                                p.content,
                            ...
                            WHERE p.id = :postId
                        """)
        Optional<PostResponseWithMedia> findPostDto(@Param("postId") UUID postId);
        ...
        @Query("""
                            SELECT new com.__blog.model.dto.response.post.PostResponse(
                                ...
                                p.content,
                                p.createdAt,
                                (SELECT m.filePath FROM Media m WHERE m.post = p ORDER BY m.displayOrder ASC LIMIT 1),
                                p.user.firstname,
                                p.user.lastname,
                                p.user.username
                            )
                            FROM Post p
                        """)
        List<PostResponse> findAllPostsPreview();

        // All media for a single post
        @Query("""
                            SELECT new com.__blog.model.dto.response.MediaResponse(
                                m.id, m.filename, m.filePath, m.fileType, m.fileSize, m.displayOrder
                            )
                            FROM Media m
                            WHERE m.post.id = :postId
                            ORDER BY m.displayOrder ASC
                        """)
        List<MediaResponse> findAllMediaByPostId(@Param("postId") UUID postId);

        // All tags for a single post
        @Query("""
                            SELECT new com.__blog.model.dto.response.TagsResponse(
                                t.id, t.tags
                            )
                            FROM Tags t
                            WHERE t.post.id = :postId
                        """)
        List<TagsResponse> findAllTagsByPostId(@Param("postId") UUID postId);
